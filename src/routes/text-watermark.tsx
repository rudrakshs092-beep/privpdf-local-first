...
export const Route = createFileRoute("/text-watermark")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Add a customizable text watermark to PDF pages locally in your browser." },
      { property: "og:title", content: "Text Watermark — PrivPDF" },
      { property: "og:description", content: "Add a customizable text watermark to PDF pages locally in your browser." },
...
function previewPosition(position: WatermarkPosition) {
  switch (position) {
    case "top-left": return "left-2 top-2";
    case "top-right": return "right-2 top-2";
...
function getPdfPosition(position: WatermarkPosition, pageWidth: number, pageHeight: number, textWidth: number, fontSize: number) {
  const margin = 24;
  switch (position) {
    ...
    default: return { x: (pageWidth - textWidth) / 2, y: (pageHeight - fontSize) / 2 };
  }
}
...
function Page() {
  ...
  const togglePage = (pageNumber: number) => {
    setSelectedPages((current) => current.includes(pageNumber) ? current.filter((page) => page !== pageNumber) : [...current, pageNumber].sort((a, b) => a - b));
  };

  const toggleAll = () => {
    setSelectedPages((current) => current.length === thumbnails.length ? [] : thumbnails.map((_, index) => index + 1));
  };
  ...
  const generate = async () => {
    ...
    if (!Number.isFinite(opacityValue) || opacityValue < 0 || opacityValue > 1 || !Number.isFinite(rotationValue) || !Number.isFinite(fontSizeValue) || fontSizeValue < 8) {
  ...
  return (
    <ToolShell title="Text Watermark" description="Add a text watermark to all or selected PDF pages. Everything is processed locally on your device.">
      <div className="space-y-6">
        {!file && !loading ? (
          <FileDrop accept="application/pdf" label="Drop your PDF here" hint="One PDF file to watermark locally." onFiles={pick} />
        ) : loading ? (
          ...
          <>
            ...
            <div className="grid gap-4 rounded-xl border border-border bg-surface-muted p-4 sm:grid-cols-2">
              <label className="text-sm font-semibold sm:col-span-2">
                Watermark text
                <Input value={text} onChange={(event) => setText(event.target.value)} maxLength={120} placeholder="CONFIDENTIAL" className="mt-2" />
              </label>
              <label className="text-sm font-semibold">
                Position
                <select value={position} onChange={(event) => setPosition(event.target.value as WatermarkPosition)} className="mt-2 flex h-10 w-full rounded-lg border border-input bg-background px-3 text-sm font-normal outline-none focus-visible:ring-2 focus-visible:ring-ring">
              ...
              <label className="text-sm font-semibold">
                ...
                <input type="range" min="5" max="100" step="5" value={opacity} onChange={(event) => setOpacity(event.target.value)} className="mt-3 block h-2 w-full cursor-pointer accent-primary" />
              </label>
              <label className="text-sm font-semibold">
                Rotation <span className="font-normal text-muted-foreground">{rotationValue}°</span>
                <input type="range" min="-180" max="180" step="5" value={rotation} onChange={(event) => setRotation(event.target.value)} className="mt-3 block h-2 w-full cursor-pointer accent-primary" />
              </label>
              <label className="text-sm font-semibold">
                Font size <span className="font-normal text-muted-foreground">{fontSizeValue} pt</span>
                <input type="range" min="8" max="96" step="2" value={fontSize} onChange={(event) => setFontSize(event.target.value)} className="mt-3 block h-2 w-full cursor-pointer accent-primary" />
              </label>
            </div>
            ...
            <div>
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">Choose every page or select specific pages.</p>
                </div>
                <Button variant="secondary" size="sm" onClick={toggleAll}>{allSelected ? "Clear all" : "Select all"}</Button>
              </div>
              <div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {thumbnails.map((url, index) => {
                  ...
                  return (
                    <button key={pageNumber} type="button" onClick={() => togglePage(pageNumber)} className={`relative overflow-hidden rounded-xl border p-2 text-left transition-colors ${selected ? "border-primary-strong bg-primary-soft" : "border-border bg-surface"}`}>
                      <div className="relative overflow-hidden rounded-lg bg-surface-muted">
                        <img src={url} alt={`Preview of page ${pageNumber}`} className="block aspect-[4/3] size-full object-contain" />
                        {selected && <span className={`absolute ${previewPosition(position)} max-w-[88%] truncate rounded bg-background/85 px-1.5 py-0.5 text-[10px] font-bold text-foreground shadow-sm`} style={{ opacity: opacityValue / 100, transform: `${position === "center" ? "translate(-50%, -50%) " : ""}rotate(${rotationValue}deg)` }}>{text.trim() || "Watermark"}</span>}
                      </div>
                      <div className="flex items-center gap-2 px-1 pt-2 text-xs font-semibold text-muted-foreground"><span>Page {pageNumber}</span>{selected && <Check className="ml-auto size-4 text-primary-strong" aria-hidden="true" />}</div>
                    </button>
                  );
                })}
              </div>
        ...
        {file && !loading && <Button onClick={generate} disabled={busy || selectedPages.length === 0}>{busy ? "Adding watermark…" : "Add watermark and download"}</Button>}
      </div>
    </ToolShell>
  );
}
